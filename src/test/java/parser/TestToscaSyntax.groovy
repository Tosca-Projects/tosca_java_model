...
class TestToscaSyntax {
	...
	@Test
	public void test() {
		def s = new ToscaSyntax()
		s.with {
			entry("service_template").is_a("map").with {
		...
		assert s.check("service_template", ToscaBuilder.simple_service_template()).OK
		assert s.check("service_template", ToscaBuilder.wrong_service_template()).OK == false
	}
}
